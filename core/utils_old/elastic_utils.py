...
class MemoryController:
    ...
    def update_last_run_states(self, input_size=None, mem_ratio=None):
        self._last_input_size = input_size
        self._last_mem_ratio = mem_ratio
...
class ElasticModule(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        if self._memory_controller is None or not torch.is_grad_enabled() or not self.training:
            ...
            self._memory_controller.update_last_run_states(input_size, mem_ratio)
        return ret
    ...
    def forward(self, *args, **kwargs):
        if self._memory_controller is None or not torch.is_grad_enabled() or not self.training:
            ...
            with self.with_mem_ratio(mem_ratio) as exact_mem_ratio:
                ret = super().forward(*args, **kwargs)
            self._memory_controller.update_last_run_states(input_size, exact_mem_ratio)
        return ret
