...
def absrel(gt, pred):
    pc_dist = torch.norm(pred-gt, dim=1)
    gt_norm = torch.norm(gt, dim=1)
    pred_norm = torch.norm(pred, dim=1)
    mask = (gt_norm > 0) & (pred_norm > 0)
    # print(gt_norm[mask].min())
    return torch.abs(pc_dist[mask] / (gt_norm[mask] + 1e-6)).mean()

def point_map_loss(gt, pred, scale):
    mask = gt[:, 3:] > 0.5
    gt = gt[:, :3] * scale.view(-1, 1, 1, 1)
    pred = pred[:, :3] * scale.view(-1, 1, 1, 1)
    weight = gt[:, 2:3]
    weight[~mask] = 1e9
    weight_norm_factor, _ = weight.view(weight.shape[0], -1).min(dim=1)
    weight = weight / weight_norm_factor.view(-1, 1, 1, 1) # normlize weight to [1, 1e9]
    loss_pre_pixel = torch.abs(gt - pred) * mask / weight
    return loss_pre_pixel.mean()